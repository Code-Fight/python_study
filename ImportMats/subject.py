...
def Start():
    table = ReadExcel()
    file_object = open('subject_sql.txt', 'w+', encoding="utf-8")
    try:
        for i in range(table.nrows-1):
            sql = '''
            insert into TB_FDN_MATERIALS_SUBJECT(CODE,NAME,TRAIN_CODE,DEP_CODE,DEP_NAME,INSERT_USER)VALUES ('%s','%s','%s','%s','%s','%s');
            ''' % (
